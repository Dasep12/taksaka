...
class _FaceVerifyStepState extends State<FaceVerifyStep>
  ...
  Future<void> _startScan() async {
    ...
    setState(() { _status = FaceVerifyStatus.scanning; _detectedFace = null; });
  ...
  void _onScanTimeout() {
    ...
    _frameTimer?.cancel();
    _scanCtrl.stop();
    _handleFailed('Wajah tidak terdeteksi dalam 10 detik. Pastikan wajah terlihat jelas dan pencahayaan cukup.');
  }

  Future<void> _onCameraFrame(CameraImage image) async {
    if (_isProcessingFrame || _status != FaceVerifyStatus.scanning) return;
    ...
    try {
      ...
      final inputImage = FaceService.instance.cameraImageToInputImage(image, cam);
      ...
      // Found face(s) — use largest one
      final face = faces.reduce((a, b) =>
          a.boundingBox.width > b.boundingBox.width ? a : b);

      if (mounted) setState(() => _detectedFace = face);

      // Check quality: head should be roughly straight
      final yaw   = (face.headEulerAngleY ?? 0).abs();
      final pitch = (face.headEulerAngleX ?? 0).abs();
      final eyeOpen = ((face.leftEyeOpenProbability ?? 0) +
          (face.rightEyeOpenProbability ?? 0)) / 2;

      if (yaw > 25 || pitch > 20) {
        // Not straight — keep trying
        _isProcessingFrame = false;
        return;
      }
      if (eyeOpen < 0.5) {
      ...
      // Good face detected — extract embedding and verify
      _frameTimer?.cancel();
      ...
      //   face,
      //   FaceService.Size(image.width.toDouble(), image.height.toDouble()),
      // );
      final embedding = FaceService.instance.extractEmbedding(
        face,
        Size(
      ...
      if (result.success) {
        setState(() { _status = FaceVerifyStatus.verified; _result = result; });
        _resultCtrl.forward();
      ...
      _handleFailed('Error: ${e.toString()}');
    } finally {
      _isProcessingFrame = false;
    }
  }
  ...
  void _handleFailed(String reason) {
    if (!mounted) return;
    setState(() { _status = FaceVerifyStatus.failed;
  ...
  void _retry() {
    _resultCtrl.reset();
    _frameTimer?.cancel();
    setState(() { _status = FaceVerifyStatus.idle; _result = null; _detectedFace = null; });
  }
                ...
                : !_cameraReady
                    ...
                    : Stack(
                        ...
                        children: [
                          ...
                          if (_detectedFace != null && _status == FaceVerifyStatus.scanning)
                            _FaceGuideBox(face: _detectedFace!,
                          ...
                          Positioned(
                            ...
                            child: Center(child: _CameraStatusBadge(
                              status: _status,
                              hasFace: _detectedFace != null,
                            )),
                          ),
...
class _FaceOvalOverlayPainter extends CustomPainter {
  ...
  void paint(ui.Canvas canvas, Size size) {
    ...
    final oval = Rect.fromCenter(center: Offset(cx, cy), width: rw * 2, height: rh * 2);
  ...
  Widget build(BuildContext context) {
    final (label, dotColor) = switch (status) {
      FaceVerifyStatus.scanning when hasFace =>
          ('WAJAH TERDETEKSI', AppColors.accent),
      FaceVerifyStatus.scanning =>
          ('MENCARI WAJAH...', AppColors.warning),
      FaceVerifyStatus.verified =>
          ('TERVERIFIKASI', AppColors.success),
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
        Icon(success ? Icons.check_rounded : Icons.close_rounded, color: color, size: 44),
  ...
  Widget build(BuildContext context) {
    return Container(color: Colors.black,
        child: const Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          SizedBox(height: 12),
          Text('Membuka kamera...', style: TextStyle(color: Colors.white70, fontSize: 13)),
        ])));
  }
}
  ...
  Widget build(BuildContext context) {
    return Container(color: Colors.black,
        ...
        child: Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          const Text('Kamera tidak tersedia', style: TextStyle(color: Colors.white70,
...
class _IdlePanel extends StatelessWidget {
  const _IdlePanel({super.key, required this.typeLabel, required this.cameraReady,
      ...
      child: Column(children: [
        Row(children: [
          ...
          const Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text('Verifikasi Wajah', style: TextStyle(fontSize: 15,
                fontWeight: FontWeight.w700, color: AppColors.primary)),
            Text('Posisikan wajah di dalam oval, lalu tekan Scan',
        ...
        SizedBox(
          ...
          child: ElevatedButton.icon(
            ...
            label: Text('Scan Wajah untuk $typeLabel',
...
class _ScanningPanel extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      padding: const EdgeInsets.all(18),
      decoration: BoxDecoration(
        color: hasFace ? AppColors.accent.withOpacity(0.08) : AppColors.primarySurface,
        borderRadius: BorderRadius.circular(18),
        border: Border.all(
            color: hasFace ? AppColors.accent.withOpacity(0.3) : AppColors.primary.withOpacity(0.15)),
      ),
      child: Row(children: [
        SizedBox(width: 28, height: 28,
            child: CircularProgressIndicator(strokeWidth: 3,
                color: hasFace ? AppColors.accent : AppColors.primary,
                backgroundColor: (hasFace ? AppColors.accent : AppColors.primary).withOpacity(0.2))),
        const SizedBox(width: 14),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(hasFace ? 'Wajah terdeteksi — memverifikasi...' : 'Mendeteksi wajah...',
              style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w700,
                  color: AppColors.grey900)),
          Text(hasFace ? 'Tetap diam sejenak' : 'Hadapkan wajah Anda ke kamera',
...
class _VerifiedPanel extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      padding: const EdgeInsets.all(18),
      decoration: BoxDecoration(color: AppColors.success.withOpacity(0.07),
          borderRadius: BorderRadius.circular(18),
      ...
      child: Row(children: [
        ...
        const SizedBox(width: 12),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          const Text('Wajah Terverifikasi ✅', style: TextStyle(fontSize: 14,
              fontWeight: FontWeight.w700, color: AppColors.success)),
          Text('Kecocokan ${(confidence * 100).toStringAsFixed(1)}% — Menyimpan absensi...',
              style: const TextStyle(fontSize: 12, color: AppColors.grey600)),
        ])),
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          Container(padding: const EdgeInsets.all(8),
              ...
              child: const Icon(Icons.face_retouching_off, color: AppColors.error, size: 20)),
          const SizedBox(width: 12),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Text('Verifikasi Gagal', style: TextStyle(fontSize: 14,
                fontWeight: FontWeight.w700, color: AppColors.error)),
            Text(reason, style: const TextStyle(fontSize: 12, color: AppColors.grey600),
